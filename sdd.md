...
Write for exactly one reader: a technical product manager — knows the project and its user stories, proficient in programming and software engineering, never reads the code.
...
List of user stories that the business logic relates to.
...
The user stories the business logic relates to (can be a reference to `## User story`)
...
Note:
- The `[required]`/`[optional]` are lables to denote whether you can omit a section
